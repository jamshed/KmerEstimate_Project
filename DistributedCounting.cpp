...
void consolidate_outputs(DistributedCount *distCount, DistributedCount &output, int maxSampleCount)
{
...
void round_robin(string &inpFile, DistributedCount &output, int k, int maxSampleCount, bool memUnconstrained)
{
    ...
    */
    ...
    DistributedCount distCount[THREAD_COUNT];
    ...
    while(true)         // read a sequence
    {
        ...
        threadIdx = (threadIdx + 1) % THREAD_COUNT;        //get to next thread
    }
    ...
    consolidate_outputs(distCount, output, maxSampleCount);
